...
**Stage: planning baseline done, build not started.** There is no application code or toolchain yet. Read the planning docs before proposing or writing any code.
...
## Toolchain
None yet (planning stage). When code lands, record the exact build / test / lint commands here and keep them current. Do not invent commands.
